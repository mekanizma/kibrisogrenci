...
import { UNI_CATALOG } from './universities.js';
import { mapPremiumFields } from './premium.js';
...
// ---------------------------------------------------------------------------
export const UNIVERSITIES = UNI_CATALOG.map((u, i) => ({
  id: u.mockId,
  slug: u.slug,
  short: u.short,
  name_tr: u.name_tr,
  ...
  lat: u.lat,
  lng: u.lng,
  // First 6 verified for mock map UX; rest still listed in the directory.
  coordinates_verified: i < 6,
  students: u.students,
}));

const PHOTOS = {
  hero: "/hero.jpg",
  building: "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=800&q=70",
  i1: "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=800&q=70",
  i2: "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?auto=format&fit=crop&w=800&q=70",
  i3: "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=800&q=70",
  i4: "https://images.unsplash.com/photo-1493809842364-82806fdbf73d?auto=format&fit=crop&w=800&q=70",
  i5: "https://images.unsplash.com/photo-1616594039964-ae9021a400a0?auto=format&fit=crop&w=800&q=70",
  i6: "https://images.unsplash.com/photo-1484154218962-a197022b5858?auto=format&fit=crop&w=800&q=70",
  i7: "https://images.unsplash.com/photo-1554995207-c18c203602cb?auto=format&fit=crop&w=800&q=70",
  i8: "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=800&q=70",
  i9: "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=800&q=70",
  i10: "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=800&q=70",
};
...
// Demo promote samples so cards show tier features out of the box
{
  const inDays = (n) => new Date(Date.now() + n * 86400000).toISOString();
  const l1 = LISTINGS.find((x) => x.id === 'l-1');
  if (l1) { l1.premium_tier = 'platinum'; l1.premium_until = inDays(25); }
  const l2 = LISTINGS.find((x) => x.id === 'l-2');
  if (l2) { l2.premium_tier = 'gold'; l2.premium_until = inDays(20); }
  const l3 = LISTINGS.find((x) => x.id === 'l-3');
  if (l3) { l3.premium_tier = 'bronze'; l3.premium_until = inDays(5); }
}
...
export function publicListing(l) {
  ...
  return {
    ...
    premium_rank: prem.premium_rank,
    premium: prem.premium,
    featured: prem.featured || !!l.featured,
  };
}
