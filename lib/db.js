...
import {
  normalizeRoommateCriteria,
  parseRoommateCriteria,
  roommateCriteriaToForm,
  mergeAmenitiesWithRoommate,
  stripRoommateTags,
} from '@/lib/roommate-criteria';

let roommateColumnReady = null;

async function listingsHaveRoommateColumn(admin) {
  if (roommateColumnReady != null) return roommateColumnReady;
  const { error } = await admin.from('listings').select('roommate_criteria').limit(0);
  if (!error) {
    roommateColumnReady = true;
    return true;
  }
  if (/roommate_criteria|does not exist|42703|schema cache/i.test(error.message || '')) {
    roommateColumnReady = false;
    return false;
  }
  roommateColumnReady = true;
  return true;
...
async function fetchRoommateCriteriaForListing(admin, listingId) {
  if (!listingId || !(await listingsHaveRoommateColumn(admin))) return null;
  const { data, error } = await admin
    .from('listings')
    .select('roommate_criteria')
    .eq('id', listingId)
    .maybeSingle();
  if (error) return null;
  return data?.roommate_criteria ?? null;
}
...
/** Attach university short names for listing cards / similar lists. */
async function attachUniversitiesToListings(admin, items) {
  const list = Array.isArray(items) ? items : [];
  if (!list.length) return list;

  const listingIds = list.map((l) => l.id).filter(Boolean);
  let links = [];
  if (listingIds.length) {
    const { data } = await admin
      .from('listing_universities')
      .select('listing_id, university_id, sort_order')
  ...
  for (const row of links) {
    ...
    byListing[row.listing_id].push(row.university_id);
  }

  const allIds = [...new Set([
    ...list.map((l) => l.uni || l.university_id).filter(Boolean),
    ...links.map((r) => r.university_id).filter(Boolean),
  ])];
  const unis = await fetchUniversitiesByIds(admin, allIds);
  const byId = Object.fromEntries(unis.map((u) => [u.id, u]));

  return list.map((l) => {
    const primaryId = l.uni || l.university_id || null;
    const linkedIds = byListing[l.id]?.length
      ? byListing[l.id]
      : (primaryId ? [primaryId] : []);
    const universities = linkedIds.map((id) => byId[id]).filter(Boolean);
    const university = (primaryId && byId[primaryId]) || universities[0] || null;
    return {
      ...l,
      university,
      universities,
...
function mapPublicListing(row, extras = {}) {
  ...
  return {
    ...
    roommate_criteria: parseRoommateCriteria(row),
    ...
    amenities: stripRoommateTags(row.amenities || []),
...
export async function dbListListings(sp) {
  ...
  items = await attachUniversitiesToListings(admin, items);
...
export async function dbGetListingByRef(ref) {
  ...
  const rawCriteria = await fetchRoommateCriteriaForListing(admin, data.id);
  if (rawCriteria != null) {
    mapped.roommate_criteria = parseRoommateCriteria({ roommate_criteria: rawCriteria });
  }
  ...
  const similarWithUni = await attachUniversitiesToListings(admin, similarMapped);
  const lookup = await priceIndexLookup([
    { ...mapped, university_id: mapped.uni },
    ...similarWithUni.map((m) => ({ ...m, university_id: m.uni })),
  ]);

  const similar = similarWithUni.map((m) => {
  ...
  let roommateUniversity = null;
  const criteriaUniId = mapped.roommate_criteria?.university_id;
  if (criteriaUniId) {
    const linked = universities.find((u) => u.id === criteriaUniId);
    if (linked) {
      roommateUniversity = linked;
    } else {
      ...
      if (ru) {
        roommateUniversity = {
          ...
          name_tr: ru.name_tr,
          name_en: ru.name_en,
          short: universityShort(ru.slug, ru.name_en, ru.name_tr),
          city: ru.city,
        };
      }
    }
  ...
  return {
    ...
    roommate_university: roommateUniversity,
...
export async function dbCreateListing(user, body) {
  ...
  if (await listingsHaveRoommateColumn(admin)) {
    row.roommate_criteria = normalizeRoommateCriteria(body);
    row.amenities = stripRoommateTags(amenities);
  }
...
export async function dbGetMyListing(user, id) {
  ...
  return {
    item: {
      ...
      ...roommateCriteriaToForm(parseRoommateCriteria(data)),
...
export async function dbUpdateListing(user, id, body) {
  ...
  if (await listingsHaveRoommateColumn(admin)) {
    patch.roommate_criteria = normalizeRoommateCriteria(body);
    patch.amenities = stripRoommateTags(amenities);
  }
...
export async function dbMyBilling(user) {
  ...
  const mine = await dbMyListings(user);
  const now = Date.now();
  const activePackages = (mine.items || [])
    .filter((l) => {
      if (!l.premium_tier) return false;
      if (!l.premium_until) return true;
      return new Date(l.premium_until).getTime() > now;
    })
    .map((l) => ({
      listing_id: l.id,
...
export async function dbAdminListings({ status, q, limit = 80, offset = 0 } = {}) {
  const admin = supabaseAdmin();
  ...
  let query = admin
    .from('listings')
    .select(
      'id, reference_code, title_tr, status, risk_flags, price_amount, price_currency, city, view_count, contact_reveal_count, premium_tier, premium_until, created_at, updated_at, landlord_profiles(display_name), listing_photos(storage_key, sort_order)',
      { count: 'exact' },
    )
  ...
  if (term) {
    ...
    if (safe) {
      query = query.or(`reference_code.ilike.%${safe}%,title_tr.ilike.%${safe}%,city.ilike.%${safe}%`);
    }
  }
...
export async function dbAdminListingAction(adminUser, body) {
  const admin = supabaseAdmin();
  const id = body?.id;
  ...
  if (action === 'approve') {
    return dbAdminReview(adminUser, { id, action: 'approve' });
  }

  const { data: existing } = await admin
    .from('listings')
    .select('id, status, reference_code, rejection_reason')
    .eq('id', id)
    .maybeSingle();
  if (!existing) return { error: 'not_found', status: 404 };
  ...
  if (action === 'delete') {
    ...
    await admin.from('audit_log').insert({
      ...
      entity_type: 'listing',
      entity_id: id,
      before_snapshot: { status: existing.status, reference_code: existing.reference_code },
    });
    return { ok: true, deleted: true };
  }
  ...
  if (action === 'unpublish') {
    const patch = {
      status: 'rejected',
      rejection_reason: reason || 'Yönetici tarafından yayından kaldırıldı.',
    };
  ...
  if (action === 'pause') {
    if (existing.status !== 'published') return { error: 'invalid_status', status: 400 };
    const { error } = await admin.from('listings').update({ status: 'paused', updated_at: new Date().toISOString() }).eq('id', id);
    if (error) throw error;
    await admin.from('audit_log').insert({
      actor_user_id: adminUser.id,
...
export async function dbAdminListingDetail(id) {
  ...
  const criteria = parseRoommateCriteria(data);
  let roommateUniversity = null;
  if (criteria.university_id) {
    const { data: ru } = await admin
      .from('universities')
      .select('id, slug, name_tr, name_en, city')
      .eq('id', criteria.university_id)
      .maybeSingle();
    if (ru) {
      roommateUniversity = {
        id: ru.id,
        slug: ru.slug,
        name: ru.name_tr || ru.name_en,
        city: ru.city,
      };
    }
  }
  return {
    item: {
      ...
      max_occupants: data.max_occupants,
      gender_preference: data.gender_preference,
      roommate_criteria: parseRoommateCriteria(data),
      available_from: data.available_from,
      minimum_stay_months: data.minimum_stay_months,
      price: { amount: Number(data.price_amount), currency: data.price_currency },
...
export async function dbAdminAudit() {
  ...
  const listingIds = [...new Set(rows.flatMap((r) => {
    const snap = r.after_snapshot || r.before_snapshot || {};
    const ids = [];
    if (r.entity_type === 'listing' && r.entity_id) ids.push(r.entity_id);
    if (snap.listing_id) ids.push(snap.listing_id);
    return ids;
  }))];
  ...
  const [{ data: profiles }, { data: listings }] = await Promise.all([
    actorIds.length
      ...
      : Promise.resolve({ data: [] }),
    listingIds.length
      ? admin.from('listings').select('id, reference_code, title_tr').in('id', listingIds)
      : Promise.resolve({ data: [] }),
  ]);
  ...
  return {
    items: rows.map((r) => {
      ...
      const listingId = after.listing_id || before.listing_id
        || (r.entity_type === 'listing' ? r.entity_id : null);
      const listing = listingId ? listingById[listingId] : null;
      const actor = r.actor_user_id ? profileById[r.actor_user_id] : null;
      return {
        id: r.id,
        action: r.action,
        ...
        listing_ref: listing?.reference_code || after.listing_ref || null,
        listing_title: listing?.title_tr || null,
        plan: after.plan || after.plan_id || before.plan || null,
        order_id: after.platform_order_id || null,
        payment_id: after.payment_id || after.shopier_payment_id || null,
        premium_until: after.premium_until || null,
        status: after.status || null,
        reason: after.reason || after.rejection_reason || null,
