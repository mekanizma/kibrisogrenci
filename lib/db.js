...
import { UNI_CATALOG, universityShort, normalizeUniversityIds } from '@/lib/universities';
...
const listingSelect = `
  ...
  contact_reveal_count, risk_flags, is_demo, location,
...
export async function dbGetConfig() {
  ...
  // Platform coverage cities: universities + cities that have live listings
  const uniCitySet = new Set(allMapped.map((u) => u.city).filter(Boolean));
  const coverageCities = [...new Set([...uniCitySet, ...listingCitySet])]
    .sort((a, b) => String(a).localeCompare(String(b), 'tr'));

  // Search city filter: prefer cities with listings, else full coverage list
  const cities = listingCitySet.size
    ? [...listingCitySet].sort((a, b) => String(a).localeCompare(String(b), 'tr'))
    : coverageCities;
...
export async function dbListListings(sp) {
  ...
  // Over-fetch when soft JS filters / geo sort remain
  const fetchLimit = needsPostFilter ? Math.min(Math.max(limit * 5, 60), MAX_LIST_LIMIT) : limit;
...
export async function dbUpdateProfile(user, body = {}) {
  ...
  if (body.avatar_key !== undefined) {
    patch.avatar_key = body.avatar_key || null;
  }
...
export async function dbAdminCoords() {
  ...
  return {
    items: (data || []).map((u) => {
      const pt = parseGeoPoint(u.campus_location) || { lat: 0, lng: 0 };
      return {
        id: u.id,
...
export async function dbAdminUsers() {
  ...
  const landlordIds = (data || []).filter((u) => u.role === 'landlord').map((u) => u.id);
  const { data: lps } = landlordIds.length
    ? await admin.from('landlord_profiles').select('id, user_id, display_name, verification_status, verification_note, verified_at').in('user_id', landlordIds)
    : { data: [] };
