...
/** Slimmer columns for cards / search grids (no long descriptions / location blob). */
const listingCardSelect = `
  ...
  neighbourhood, city, published_at, last_confirmed_available_at, view_count,
  listing_photos ( storage_key, sort_order ),
  landlord_profiles ( id, display_name, is_agency, verification_status, user_id )
`;
...
export async function dbListListings(sp) {
  ...
  const needsPostFilter = verifiedOnly || maxWalk > 0;
  ...
  // Over-fetch a bit when soft JS filters remain; otherwise page at DB.
  const fetchLimit = needsPostFilter ? Math.min(limit * 4, MAX_LIST_LIMIT) : limit;
...
export async function dbGetListingByRef(ref) {
  ...
  const [uniRes, histRes, similarRes] = await Promise.all([
  ...
  const similarMapped = (similarRes.data || []).map((s) => mapPublicListing(s));
