...
export function isMockMode() {
  // Live-by-default: mock only when explicitly enabled — never in production.
  if (process.env.MOCK_MODE !== 'true') return false;
  if (process.env.NODE_ENV === 'production') {
    console.error(JSON.stringify({ level: 'error', msg: 'MOCK_MODE ignored in production' }));
    return false;
  }
  return true;
}

/** Dev-only demo auth bypass — always false in production. */
export function allowMockDemoAuth() {
  if (process.env.NODE_ENV === 'production') return false;
  return process.env.MOCK_ALLOW_DEMO_AUTH === 'true';
}
